"""Excel exporter preserving the legacy workbook's safety guarantees."""
...
def export_legacy_payload(
    ...
    *,
    ...
    """Append one legacy payload while preserving the original workbook."""

    if sheet not in {"中文", "英文"}:
        raise ValueError("sheet must be 中文 or 英文")
    workbook = workbook or workbook_from_environment()
    if workbook is None:
        raise ValueError(
    ...
    try:
        ...
        appended_row = None
        if not schema_only and payload is not None:
            duplicate = find_duplicate_row(wb[sheet], payload)
            if duplicate is not None:
                return ExportResult(
            ...
            appended_row = append_payload(wb[sheet], payload)

        if not schema_changed and appended_row is None:
            return ExportResult(action="schema_already_current", workbook=str(path))

        excluded_before = {name: before[name] for name in before}
        if appended_row is not None and appended_row <= wb[sheet].max_row:
            original = load_workbook(path, read_only=False, data_only=False)
            try:
                excluded_before[sheet] = sheet_snapshot(original[sheet], {appended_row})
            finally:
                original.close()
        ...
        backup = save_validate_replace(
            ...
            wb,
            excluded_before,
            sheet if appended_row is not None else None,
            appended_row,
        )
        return ExportResult(
            ...
            workbook=str(path),
            backup=str(backup),
            sheet=sheet if appended_row is not None else None,
            row=appended_row,
        )
    finally:
        wb.close()
...
def legacy_main(argv: list[str] | None = None) -> int:
    """Maintain the original script contract during the Core migration."""
